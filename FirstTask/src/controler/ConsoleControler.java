...
public class ConsoleControler implements Controler{
	...
	public void countCost() {
		view.renderMessage("Deritative cost:"
				+ deritative.CountCost());
	...
	public void filterDeritative() {
		...
		case 1:{
			...
			view.renderObligations(
					fImpl.filter(new PercentsFilter(min, max))
					);			
			break;
		}
		case 2:{
			...
			view.renderObligations(
					fImpl.filter(new PropertyFilter(min, max))
					);			
			break;
		}
		case 3:{
			...
			double max=scaner.nextDouble();
			view.renderObligations(
					fImpl.filter(new RiskFilter(min, max))
					);			
			break;
		}
		case 4:{
